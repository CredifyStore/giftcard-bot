...
def delete_product_value(product_key: str, face_value_cents: int):
    """Remove um valor de produto e todos os códigos associados não usados."""
    conn = get_conn()
    # Pega o amount_cents correspondente
    row = conn.execute(
        "SELECT amount_cents FROM product_values WHERE product_key=? AND face_value_cents=?",
        (product_key, face_value_cents)
    ).fetchone()
    if row:
        amount_cents = row["amount_cents"]
        # Remove códigos não usados
        conn.execute(
