...
async def show_catalog(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for key, item in CATALOG.items():
        total = sum(count_gift_codes(key, v["amount"]) for v in item["values"])
        kb.append([InlineKeyboardButton(
            f"{item['name']} ({total} Disponíveis)",
...
async def show_product(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Lista de estoque por valor
    lines = [f"{item['emoji']} *{_esc(item['name'])}* \\- Estoque disponível\n"]
    for v in item["values"]:
        qty = count_gift_codes(product_key, v["amount"])
        if qty > 0:
            lines.append(f"R\\$ {v['face_value']//100},00 \\- {qty} Unidades Disponíveis")

    lines.append(f"\n💰 Seu saldo: *R\\$ {brl2(balance)}*")
    lines.append(f"Escolha o valor \\(face / preço com {discount_pct}% OFF\\):")
    ...
    for i, v in enumerate(item["values"]):
        qty   = count_gift_codes(product_key, v["amount"])
        face  = v["face_value"] // 100
        price = brl2(v["amount"])

        if qty == 0:
            btn = InlineKeyboardButton(f"❌ R$ {face},00", callback_data="no_stock")
        elif balance < v["amount"]:
            btn = InlineKeyboardButton(f"🔴 R$ {face} (R$ {price})", callback_data=f"insuf:{product_key}:{v['amount']}")
        else:
            btn = InlineKeyboardButton(f"R$ {face} (R$ {price})", callback_data=f"buy:{product_key}:{v['amount']}")
...
# ──── Saldo insuficiente (mensagem detalhada) ─────────────────

async def insufficient_balance(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    parts       = query.data.split(":")          # insuf:product_key:amount
    ...
    await query.edit_message_text(
        ...
        f"Adicione saldo via PIX para continuar comprando\\:",
...
async def initiate_buy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if gift_code:
        ...
        text = (
            ...
            f"💳 Valor do pin: R\\$ {face_value//100},00\n"
            f"💸 Você pagou: R\\$ {brl2(amount_cents)} \\({discount_pct}% OFF\\)\n"
...
MIN_TOPUP     = 3000    # R$ 30 mínimo conforme referência
MAX_TOPUP     = 100000  # R$ 1000 máximo
...
async def topup_custom(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # Mantido por compatibilidade — redireciona para pix_info
...
async def topup_receive_custom(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Valor inválido\\. Digite apenas números\\. Ex: `85`", parse_mode=ParseMode.MARKDOWN_V2)
        return
...
async def _generate_topup_pix(update, ctx, amount_cents: int, via_query=None):
    ...
    try:
        ...
        msg = f"⚠️ Erro ao gerar PIX\\. Tente novamente ou contate o suporte\\.\n\n`{_esc(str(e))}`"
        if via_query:
    ...
    # Envia QR code primeiro como imagem
    ...
    # Depois envia o texto com copia e cola
    text = (
