...
Correções aplicadas:
- _home_text() extrai o texto duplicado de start/back_to_start
- show_product usa count_all_gift_codes() — uma query ao invés de N
- MIN_TOPUP_CENTS importado do config em vez de redefinido
- Imports limpos (MIN_TOPUP_CENTS vem do config)
- _esc e brl2 vêm de utils.py
"""
import base64
import datetime
...
from models.database import (
    ...
    count_all_gift_codes, pop_gift_code,
...
# Aliases locais para manter compatibilidade com o resto do código
_esc  = esc
brl2  = brl

MAX_TOPUP = 100_000  # R$ 1.000,00
...
def _home_text(user) -> str:
    """Texto do menu principal — centralizado para evitar duplicação."""
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        _home_text(user), reply_markup=_main_kb(), parse_mode=ParseMode.MARKDOWN_V2,
    )
...
async def show_product(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Uma única query para todos os estoques do produto
    ...
    if not kb:
        kb = [[InlineKeyboardButton("😔 Sem estoque disponível", callback_data="catalog")]]
...
async def insufficient_balance(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    amount_need = int(query.data.split(":")[2])
    balance     = get_balance(query.from_user.id)
    await query.edit_message_text(
...
async def initiate_buy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if gift_code:
        ...
        text = (
            ...
            f"▫️ Certifique\\-se de resgatar dentro do prazo\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"💰 Saldo Atual: R\\$ {brl2(new_balance)}\n"
            f"Obrigado por comprar conosco\\! 🚀"
        )
    else:
        # Sem código: estorna imediatamente
...
async def topup_pix_info(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    min_brl = MIN_TOPUP_CENTS // 100
    max_brl = MAX_TOPUP // 100
    await query.edit_message_text(
        f"💠 *Recarga via PIX*\n\n"
        f"Digite o valor que deseja adicionar\\.\n"
        f"📌 Mínimo: *R\\$ {min_brl},00*\n"
        f"📌 Máximo: *R\\$ {max_brl},00*\n"
...
async def my_orders(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for o in orders:
        ...
        if o["status"] == "delivered" and o.get("gift_code"):
            ...
            lines.append(
                f"{ico} *{_esc(o['product_name'])}* \\- {_esc(o['status'])}\n"
                f"🆔 `{o['id']}`\n"
            )
...
async def help_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"💸 Todos os pins com *{int(DISCOUNT*100)}% de desconto*\n\n"
