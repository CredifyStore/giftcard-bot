"""
Handlers do usuário — DM completa:
Menu principal, catálogo, carteira, recarga, pedidos,
perfil, ranking top 10.
"""
...
async def wallet_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    from models.database import get_wallet_txns
    txns = get_wallet_txns(query.from_user.id, limit=5)

    lines = [f"👛 *Sua Carteira*\n\n💰 Saldo disponível: *R\\$ {balance/100:.2f}*"]
    if txns:
        ...
        for t in txns:
            ...
            lines.append(f"{ico} {sign}R\\$ {t['amount_cents']/100:.2f} — {_esc(t['description'] or '')}")
...
async def topup_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    kb = [[InlineKeyboardButton(f"R$ {a/100:.0f}", callback_data=f"topup_amount:{a}")]
...
async def topup_receive_custom(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ Valor inválido. Use /start e tente novamente.")
        return
    if amount_cents < MIN_TOPUP_CENTS:
        await update.message.reply_text(
            f"❌ Mínimo de recarga: R\\$ {MIN_TOPUP_CENTS/100:.2f}\\.",
            parse_mode=ParseMode.MARKDOWN_V2,
        )
        return
...
async def _generate_topup_pix(update, ctx, amount_cents: int, via_query=None):
    ...
    try:
        ...
        msg = f"⚠️ Erro ao gerar PIX: `{e}`"
        if via_query:
            await via_query.edit_message_text(msg, parse_mode=ParseMode.MARKDOWN)
        else:
...
async def initiate_buy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if gift_code:
        ...
        text = (
            ...
            f"💳 Saldo do pin: R\\$ {face_value/100:.0f}\n"
            f"💸 Pago: R\\$ {amount_cents/100:.2f} \\({discount_pct}% OFF\\)\n"
            f"💰 Saldo restante: *R\\$ {new_balance/100:.2f}*\n\n"
            f"🔑 *Seu código:*\n`{_esc(gift_code)}`\n\n"
            f"Obrigado pela compra\\! 🙏"
        )
    else:
        # Sem estoque — estorno automático
        ...
        text = (
            ...
            f"Seu saldo de *R\\$ {amount_cents/100:.2f}* foi estornado automaticamente\\.\n\n"
            f"💰 Saldo atual: *R\\$ {(new_balance + amount_cents)/100:.2f}*"
        )
...
async def my_orders(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not orders:
        await query.edit_message_text("📭 Nenhum pedido ainda.", reply_markup=InlineKeyboardMarkup(kb))
        return
    ...
    for o in orders:
        ...
        # Mostra pin disponibilizado se entregue
        if o["status"] == "delivered" and o.get("gift_code"):
            lines.append(
                f"{ico} `{o['id']}` — *{_esc(o['product_name'])}* R\\$ {face/100:.0f}\n"
                f"   🔑 `{_esc(o['gift_code'])}`"
            )
        else:
...
async def my_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"📛 Username: {_esc(tag)}\n\n"
        f"💰 Saldo atual: *R\\$ {profile['balance_cents']/100:.2f}*\n"
        f"🛒 Total de compras: *{profile['total_purchases']}*\n"
        f"💸 Total gasto: *R\\$ {profile['total_spent']/100:.2f}*"
    )
    kb = [[InlineKeyboardButton("🔙 Menu", callback_data="start")]]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN_V2)
