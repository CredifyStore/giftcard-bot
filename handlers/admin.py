...
from models.database import (
    ...
    list_wallets_with_balance, refund_wallet, get_balance,
    get_user_orders,
)
...
async def _send_admin_panel(send_fn):
    kb = [
        ...
        [InlineKeyboardButton("↩️ Estornar saldo",       callback_data="adm_refund_start")],
    ]
...
refund_conv = ConversationHandler(
    entry_points=[CallbackQueryHandler(adm_refund_start, pattern="^adm_refund_start$")],
    states={
        REFUND_USER:    [MessageHandler(filters.TEXT & ~filters.COMMAND, adm_refund_user)],
        REFUND_AMOUNT:  [MessageHandler(filters.TEXT & ~filters.COMMAND, adm_refund_amount)],
        REFUND_CONFIRM: [CallbackQueryHandler(adm_refund_confirm, pattern="^adm_refund_confirm$")],
    },
    fallbacks=[CallbackQueryHandler(adm_cancel, pattern="^adm_panel$")],
    per_user=True, per_chat=False,
)
