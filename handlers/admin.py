...
# ──── Excluir valor de produto ────────────────────────────────

async def adm_delete_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Lista produtos para o admin escolher qual valor excluir."""
    query = update.callback_query
    if not is_admin(query.from_user.id):
        await query.answer("🚫", show_alert=True); return
    await query.answer()
...
async def adm_delete_pick_product(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Mostra os valores do produto escolhido para excluir."""
    query = update.callback_query
    if not is_admin(query.from_user.id):
        await query.answer("🚫", show_alert=True); return
    await query.answer()
    ...
    if not product or not product["values"]:
        await query.edit_message_text("❌ Produto sem valores cadastrados\\.",
                                       parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    for v in product["values"]:
        ...
        label = f"R$ {v['face_value_cents']//100},00 → R$ {brl2(v['amount_cents'])} ({qty} un)"
        kb.append([InlineKeyboardButton(
            f"🗑 {label}",
...
async def adm_delete_value_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    from models.database import delete_product_value
    ...
    await query.edit_message_text(
        f"✅ Valor *R\\$ {face_value_cents//100},00* removido de *{_esc(name)}*\\.\n\n"
        f"Os códigos não utilizados deste valor também foram removidos\\.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin", callback_data="adm_panel")]]),
        parse_mode=ParseMode.MARKDOWN_V2,
    )
